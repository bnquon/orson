...
export type PendingScenarioAction =
  { kind: 'select'; id: string } | { kind: 'import' } | { kind: 'new' };
...
export function executeScenarioAction(
  ...
  callbacks: { importScenario(): void; selectScenario(id: string): void; newScenario(): void },
): void {
  if (action.kind === 'import') callbacks.importScenario();
  else if (action.kind === 'select') callbacks.selectScenario(action.id);
  else callbacks.newScenario();
}

export function decideScenarioAction(input: {
  kind: 'import' | 'select' | 'new';
  runActive: boolean;
  ...
  if (input.runActive || input.selectionLoading) return 'blocked';
  if (input.kind === 'select' && input.target?.id === input.currentScenarioId) return 'proceed';

  const canReplaceDraft = input.kind !== 'select' || input.target?.status !== 'invalid';
